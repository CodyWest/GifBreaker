...
def final ():
    ...
    if request.form['moshtype'] == 'overlay':
        ...
        fileObject.close()
        MoshinSomeStuff.overlay('upload1','upload2','youroverlay.avi')
        flask.send_file('youroverlay.avi')
    elif request.form['moshtype'] == 'bloom':
        duration= int(request.form['duration'])
        wait = int(request.form['wait'])
        MoshinSomeStuff.bloom('upload1','yourbloom.avi',wait,duration)
        flask.send_file('yourbloom.avi')
    elif request.form['moshtype'] == 'shmear':
        MoshinSomeStuff.shmear('upload1','yourshmear.avi')
        flask.send_file('yourshmear.avi')
